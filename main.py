...
def export_report(duration):
    ...
    myMD.MD_header(1, "About")
    myMD.MD_text("""This Report was autogenerated by `FSizer` tool, developed by `Wesam Assarraj`.""", True)
    myMD.MD_text("""For any further information or requests you can reach him on [w.h.assarraje@gmail.com].""", True)
    myMD.MD_horizontal_rule()

    myMD.MD_header(2, "Computer General Information")
    table = myReport.get_pc_information()
    myMD.MD_table(table)

    myMD.MD_header(2, "Computer Partitions Information")
    table = myReport.get_logical_disk()
    myMD.MD_table(table)

    myMD.MD_header(2, "Data Growth Rate")
    table, unit = myReport.get_growth(duration)
    filename = myReport.draw_growth(table,
                                    unit,
                                    myMD.MD_getReportpath(),
                                    myMD.MD_getFoldername())

    myMD.MD_image(None, filename, True)

    for row in table:
        row["Size"] = "{0} {1}".format(row["Size"], unit)

    myMD.MD_table(table)

    myMD.MD_header(3, "Actual Shared Paths")
    myMD.MD_table(myReport.actual_shared_paths())

    myMD.MD_header(3, "Shared paths - More Details")
    myMD.MD_table(myReport.shared_paths_information(duration))

    #myMD.MD_header(3, "Data Types")
